...
GlazeWM の `window_rules` の ignore 一覧には、管理対象から外すアプリケーションが並びます。[このマシン](/ja/machine/)では 11 プロセスあり、いくつかは**同名のリポジトリに直結**します。つまりこの一覧は「どのウィンドウを無視するか」と同じだけ「どんなプロジェクトが存在するか」を語っていました。公開リポジトリに置くには多弁です。
...
スクリプトは生成物の隣に symlink されます（キーバインドから安定したパスで届かせるため）。そして `$PSScriptRoot` は**リンク自身のディレクトリ**、つまり出力がある場所を指します。そこから base を読めば、生成物を自分自身に食わせることになります。リンク経由の初回実行でこのエラーが出たので、スクリプトは自分の symlink を辿って源を探すようになっています。
